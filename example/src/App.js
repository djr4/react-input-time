...
const App = () => {
  return <div style={{ margin: '64px auto', width: 600, }}>
    ...
    <InputTime
      ...
      value={`${new Date().getHours()}:${new Date().getMinutes() < 10 ? `0${new Date().getMinutes()}` : new Date().getMinutes()}`}
      label={'Time Now? '}
      />
    <p>
...
import { InputTime } from 'input-time'
  ...
  />`
    ...
    <div style={{ display: 'flex'}}>
      <div><InputTime onChange={(e) => { console.log(e)}} iconRight value={`07:00`} label={`Opening hours`} labelColor="green" /></div>
  ...
  />
    ...
    <div><InputTime value={'21:30'} componentStyle={{ width: 45, }} inputStyle={{ width: 20, }} /></div>
